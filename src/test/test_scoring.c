...
static void add_move(uint64_t board_id, uint64_t player_id, bool is_white_move) {
...
static bool scores_are_close(double a, double b) {
...
static bool test_win_proportional_payout() {
    ...
    for(int i=0; i<18; i++) add_move(1, 1, true);
    for(int i=0; i<2; i++) add_move(1, 2, true);
    for(int i=0; i<20; i++) add_move(1, 3, false);
...
static bool test_draw_proportional_payout() {
    ...
    for(int i=0; i<10; i++) add_move(1, 1, true);
    for(int i=0; i<10; i++) add_move(1, 2, false);
...
static bool test_dual_side_contribution_halved_payout() {
    ...
    for(int i=0; i<10; i++) add_move(1, 1, true);
    for(int i=0; i<10; i++) add_move(1, 1, false);
    for(int i=0; i<10; i++) add_move(1, 2, true);
...
static bool test_no_payout_for_zero_contribution() {
    ...
    for(int i=0; i<10; i++) add_move(1, 1, true);
    for(int i=0; i<10; i++) add_move(1, 3, false);
...
static const TestCase cases[] = {
    ...
    {"dual side contribution halved payout", test_dual_side_contribution_halved_payout},
