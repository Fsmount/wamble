...
static int board_capture_worker_context(const WambleQueryService **out_qs,
                                        char *profile_name,
                                        size_t profile_name_len,
                                        char *profile_conn,
                                        size_t profile_conn_len,
                                        char *global_conn,
                                        size_t global_conn_len) {
  ...
  if (!cfg || !qs || !profile_name || profile_name_len == 0 ||
      db_format_connection_string(cfg, 0, profile_conn, profile_conn_len) != 0 ||
...
static void *board_supply_refresh_worker(void *arg) {
  ...
  if (!state)
    return NULL;
  int context_ok =
      db_set_global_store_connection(state->global_conn) == 0 &&
  ...
  int lists_ok = context_ok && dormant.status == DB_OK && active.status == DB_OK &&
...
static int board_scoring_job_flush(BoardScoringJob *job) {
  ...
  int rc = wamble_persistence_flush_buffer(
      job->intents, qs, job->max_batches, job->max_intents,
...
static void board_scoring_enqueue_async(uint64_t board_id, GameResult result) {
  ...
  if (board_capture_worker_context(
          &job->qs, job->profile_name, sizeof(job->profile_name),
          job->profile_conn, sizeof(job->profile_conn), job->global_conn,
